...
class themes:
    ...
    def get_mal_ids(search) -> "malids list":
        return requests.get(
            f"https://themes.moe/api/anime/search/{search.replace(' ', '%20')}"
        ).json()
    ...
    def get_info_from_ids(self, ids, get_input=False):
        animeInfo = requests.post(
            "https://themes.moe/api/themes/search", json=ids
        ).json()
        data_dict = {}
        info_dict = {}
        print_list = []
        headers = ['S.no', 'malID', 'name', 'year', 'season']
        for index, i in enumerate(animeInfo):
            info = {}
            curr_anime_data = []
            if get_input == True:
                curr_anime_data.append(index)
            for j, k in i.items():# k is from headers[1: ]
                if type(k) != list:
                    if get_input == True:
                        # print(j,k)
                        curr_anime_data.append(k)
                    else:
                        pass
                    info[j] = k
                    ...
                    for l in k:
                        data.append(
                            [
                                l["themeType"],
                                f"{l['themeType']} {l['themeName']}.mp3",
                                l["mirror"]["mirrorURL"],
                            ]
                        )
                    data_dict[index] = data
            info_dict[index] = info
            print_list.append(curr_anime_data)
        if get_input==True:
    ...
    def get_and_print(self):
        return themes.get_info_from_ids(
            self, themes.get_mal_ids(self.keyword), get_input=True
        )
...
class themes_wrapper:
    ...
    def cover(self):
        return requests.get(
            f"https://animethemes-api.herokuapp.com/api/v1/anime/{self.malID}"
        ).json()["cover"]

    def change_video_to_audio(self):
        for j, i in enumerate(self.links):
            self.links[j][2] = themes_wrapper.get_music_from_video(self, i[0], i[2])
            self.links[j].pop(0)
        self.links.append(["cover.jpg", self.cover])

    def get_music_from_video(self, themetype, video):
        response = requests.post(
            f"https://themes.moe/api/themes/{self.malID}/{themetype}/audio",
            json=[video],
        )
